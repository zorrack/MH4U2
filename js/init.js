...
$( document ).ready(function() {
    ...
    init(map, sidebar, () => {
        $.when(facilitiesJson).done(function() {
            createFilters(facilitiesForFiltering, map);
 //           getUniqueValues(facilitiesJson, ac1);
 //           createLayers();
 //           addCategoryOverlays();
        })
    });
...
function googleSheetsToJson(data, map, sidebar) {
    facilitiesJson = new L.GeoJSON(null);
    facilitiesForFiltering = {
    ...
    for (var row in data) {
        ...
        if (marker) {
            ...
            facilitiesForFiltering.features.push(marker.feature);
