...
function getData(tabletop) {

    let sheets = (tabletop.sheets());
    createDataTypes(dataTypesTemplate, sheets);

    let mappingData = dataTypesTemplate.find(sheet => sheet.type === "[add]"); 
    let configData = dataTypesTemplate.find(sheet => sheet.name === "[conf]");
    console.log("data for mapping: " + mappingData);
